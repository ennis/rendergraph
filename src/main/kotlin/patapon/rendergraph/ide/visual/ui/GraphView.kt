package patapon.rendergraph.ide.visual.ui
...
{
    companion object {
        val LOG = Logger.getInstance(GraphEditorSkin::class.java)
    }
    ...
    private fun redrawGraph() {
        ...
        if (m != null) {
            println("Redrawing graph. ${m.nodes.size} nodes to draw")
    ...
    private fun installModelListeners(model: GraphModel<N>)
    {
        model.nodes.addListener(modelNodeChangeListener)
        model.connections.addListener(modelConnectionChangeListener)
    }

    init {
        if (control.model != null) {
            installModelListeners(control.model!!)
        }
        control.modelProperty().addListener { observable, oldValue, newValue ->
            if (oldValue != null) {
                oldValue.nodes.removeListener(modelNodeChangeListener)
                oldValue.connections.removeListener(modelConnectionChangeListener)
            }
            if (newValue != null) {
                installModelListeners(newValue)
            }
            redrawGraph()
        }
